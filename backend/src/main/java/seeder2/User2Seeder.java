...
public class User2Seeder extends Seeder2Support {
    ...
    public void seed(HBaseCustomClient client) throws IOException {
        ...
        seedUser(client, "USR002", "Operator Teknik Informatika", "operator_ti", "operator.ti@kampus.ac.id", "2",
                SP_TI_ID, SP_TI_NAME);
        seedUser(client, "USR003", "Operator Sistem Informasi Bisnis", "operator_sib", "operator.sib@kampus.ac.id",
                "2", SP_SIB_ID, SP_SIB_NAME);
        seedUser(client, "USR004", "Budi Raharjo", "budi_raharjo", "budi.raharjo@kampus.ac.id", "3", SP_TI_ID,
                SP_TI_NAME);
