...
const AdduserForm = ({
  ...
  const [isNewPassword, setIsNewPassword] = useState(!currentRowData?.id); // True jika sedang menambah user baru
  ...
  useEffect(() => {
    ...
    if (currentRowData) {
      // Jika edit user, kosongkan field password dan tandai sebagai "Tidak diubah"
      form.setFieldsValue({
        ...
        roles: currentRowData.roles,
      });
      setIsNewPassword(false); // Default tidak mengubah password saat edit
    } else {
      setIsNewPassword(true); // Default mengisi password saat tambah baru
    }
  }, [currentRowData, form]);
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (currentRowData && !isNewPassword) {
          ...
          {currentRowData && (
            <Col xs={24} sm={24} md={24}>
              <Form.Item name="changePassword" valuePropName="checked">
                <Checkbox onChange={(e) => setIsNewPassword(e.target.checked)}>
                  Ubah Password
                </Checkbox>
              </Form.Item>
            </Col>
          ...
          {(isNewPassword || !currentRowData) && (
            <Col xs={24} sm={24} md={12}>
              <Form.Item
                ...
                rules={[
                  {
                    required: isNewPassword || !currentRowData,
